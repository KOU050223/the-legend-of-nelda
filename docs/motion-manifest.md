...
| `moving`        | `ACTIVE` 中で移動入力中、または位置が前フレームから変わった |
| `dodging`       | `ACTIVE` 中で無敵時間が残っている     |
...
プレイヤー3体は歩行・攻撃・回避・睡眠を、ボスは待機・歩行・攻撃・回避・睡眠を
マニフェストへ登録している。プレイヤーの位置は毎フレームObject3Dへ同期するが、
前フレームとの差分をモーション条件へ使うため、移動中も条件が変わる境界だけが
Reactの再レンダーになる。
