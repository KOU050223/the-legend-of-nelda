...
寝ようとしている味方の近くで、**両手を閉じて胸の前で合わせる（合掌）**。
合掌を保持している間、「寝るな！」の連打を一定間隔で送る。

### 判定

- 蘇生対象の近くにいる（ゲーム側で距離を判定）
- 両手が明示的に閉じている
- 両手首の2次元距離が一定値以下である
- 合掌を保持している間、一定間隔でREVIVEを送る

ことでREVIVE進行度を増加させる。

入力認識はクライアント側で行い、ネットワークへはREVIVEアクションだけを送る。
入力間隔には通信揺らぎを吸収するため、ゲーム側の最小受付間隔よりわずかな余裕を持たせる。
