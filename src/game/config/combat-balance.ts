 ...
 * 追撃を入れた回数だけ加算される。
 *
 * 0 にしてある。仕様 §17 の攻撃順は本戦7手の反撃だけでボスHP100を
 * ちょうど削り切る組み立てで、追撃の入る余地が残っていないため。
 *
 * 本戦1つ目の布団 (6手目) のカウンターが通った時点で残りHPは20。
 * 最終ふかふか布団 (7手目) を「FINISHさせてもよい」(§4) 技として
 * 成立させるには、その手前の7手目 (枕 or あくび、10〜15) を抜けた時点で
 * HPが残っていなければならない。つまり追撃の合計が5未満に収まる必要がある。
 * §11 の大ダウンは約2.5〜3秒あり2〜3発入るので、1発あたり1以下でないと
 * 成立しない。単価をいくつに下げても仕様のタイムラインへは収まらないので、
 * 値の調整ではなく0にしている。
 *
 * 大ダウンそのものは演出として残り、追撃入力も WHIFF にはならない。
 * 追撃にダメージを持たせる場合は §17 の攻撃順かボスHPの見直しとセットで行う。
 */
export const BOSS_DOWN_FOLLOW_UP_DAMAGE = 0;
