 ...
 * 追撃を入れた回数だけ加算される。
 *
 * 1 にしてある。仕様 §17 の攻撃順は本戦7手の反撃だけでボスHP100を
 * ほぼ削り切る組み立てで、追撃に使える余地がごく狭いため。
 *
 * 本戦1つ目の布団 (5手目) のカウンターが通った時点で残りHPは20。
 * 最終ふかふか布団 (7手目) を「FINISHさせてもよい」(§4) 技として
 * 成立させるには、その手前の6手目 (枕 or あくび) を抜けた時点で HP が
 * 残っていなければならない。6手目があくびを引くと残りHPは5まで落ちるので、
 * 大ダウン中に入る追撃の合計はそこへ収まる必要がある。
 *
 * 1 が上限であることは値を振って実測した。2 以上にすると最終布団の手前で
 * ボスが落ち、ランダム枠と最終布団が発火しない。上限はテストで固定してある
 * (combat-session.test.ts の「大ダウン中に追撃し続けても最終ふかふか布団まで
 * 到達する」が単価2以上で落ちる)。
 *
 * 単価を上げる場合は §17 の攻撃順かボスHPの見直しとセットで行う
 * (§5 の前文が「初期値や細かい数値はプレイテストで調整する」としている範囲)。
 */
export const BOSS_DOWN_FOLLOW_UP_DAMAGE = 1;
